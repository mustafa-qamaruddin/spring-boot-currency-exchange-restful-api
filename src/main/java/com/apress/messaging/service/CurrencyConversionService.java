...
public class CurrencyConversionService {
    ...
    public Rate[] calculateByCode(
        ...
        Rate baseRate = rates.stream().filter(
            rate -> rate.getCode().equals(code)
        ...
        return Stream.concat(
            rates.stream().filter(
                n -> !n.getCode().equals(code)
            ).map(
                n -> new Rate(
                    n.getCode(),
                    n.getRate()/baseRate.getRate(),
                    date
                )
            ),
            Stream.of(
                new Rate(
                    CurrencyExchange.BASE_CODE,
                    1/baseRate.getRate(),
                    date
                )
            )
    ...
    public void saveRates(
        ...
        Arrays.stream(rates).forEach(
            rate -> repository.save(
                new Rate(
                    rate.getCode(),
                    rate.getRate(),
                    date
